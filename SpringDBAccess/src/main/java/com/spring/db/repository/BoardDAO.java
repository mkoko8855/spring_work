...
public class BoardDAO implements IBoardDAO {
	...
	class BoardMapper implements RowMapper<BoardVO> {
		...
		public BoardVO mapRow(ResultSet rs, int rowNum) throws SQLException {
			return new BoardVO(rs.getInt("board_no"), rs.getString("writer"), rs.getString("title"),
					rs.getString("content"), rs.getTimestamp("reg_date").toLocalDateTime());
		}
	}

	// 인서트 진행을 위해 변수하나선언해주자
	@Autowired
	...
	public BoardVO getArticle(int bno) {
		...
		return template.queryForObject(sql, new BoardMapper());
	}
