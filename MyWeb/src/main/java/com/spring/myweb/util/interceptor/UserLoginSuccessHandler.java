...
public class UserLoginSuccessHandler implements HandlerInterceptor {
	...
	//postHandle은 컨트롤러를 나갈 떄 공통 처리해야 할 내용을 작성하게 된다.
	@Override
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler, //이 메서드는 자동으로 호출이 된다.
		...
		if(request.getMethod().equals("POST")) { //겟메서드의 리턴타입이 스트링이니 equals를 사용했다.
			ModelMap map = modelAndView.getModelMap(); //getModelMap은 Model객체를 꺼내는 메서드이다.즉, 모델 만 꺼낼 것이다.
			//이제 모델한테 값달라하자(꺼내달라하자)
			UserVO vo = (UserVO) map.get("user"); //모델 내에 user라는 이름의 데이터 꺼내기. 모델은 모든 걸 다 받으니 모든걸 리턴할 수 있어야 한다. 그래서 리턴 타입이 UserVO타입이다.
			...
			if(vo != null) { //로그인 성공
				...
				session.setAttribute("login", vo);//로그인이라는 거에 vo를 저장한다.
